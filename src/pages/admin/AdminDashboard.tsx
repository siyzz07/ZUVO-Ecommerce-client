...
import { 
    Trash2, Edit2, ShoppingBag, DollarSign, Activity, 
    ...
    return (
        ...
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                {[
                    { label: 'Total Products', value: products.length, icon: ShoppingBag, color: 'text-brand-primary', bg: 'bg-brand-primary/10' },
                    { label: 'Total Revenue', value: '₹14.2M', icon: DollarSign, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
                    { label: 'Store Status', value: 'Active', icon: Activity, color: 'text-orange-500', bg: 'bg-orange-500/10' },
                ].map((stat, i) => (
                    <div 
                        key={i}
                                            ...
                                            >
                                                <td className="p-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className="font-bold text-zinc-900 dark:text-white text-sm">{p.name || 'Unnamed'}</span>
                                                    </div>
                                                </td>
                                                <td className="p-4">
                                                ...
                                                <td className="p-4">
                                                    <span className="font-bold text-zinc-900 dark:text-white">₹{p.price?.toLocaleString()}</span>
                                                </td>
                                                <td className="p-4">
                        ...
                        {filteredProducts.map((p) => (
                            ...
                            >
                                ...
                                <div className="space-y-3">
                                    <div>
                                        <h4 className="font-bold text-lg text-zinc-900 dark:text-white truncate">{p.name}</h4>
                                        <p className="text-xs text-zinc-500 mt-0.5">{p.category}</p>
                                    </div>
                                    <div className="flex items-center justify-between pt-3 border-t border-zinc-100 dark:border-zinc-800">
                                        <p className="font-bold text-brand-primary">₹{p.price?.toLocaleString()}</p>

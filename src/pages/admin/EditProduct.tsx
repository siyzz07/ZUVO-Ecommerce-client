        ...
        <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-800 shadow-xl">
            ...
            <div className="space-y-4">
                <div>
                    ...
                    {formData.category && <p className="text-brand-primary font-bold text-xs uppercase mt-1">{formData.category}</p>}
                </div>
                ...
                <div className="flex items-end justify-between pt-4 border-t border-zinc-100 dark:border-zinc-800">
                    <div>
                         ...
                         <p className="text-3xl font-bold text-zinc-900 dark:text-white">₹{formData.price || '0'}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                <form onSubmit={handleSubmit} className="flex-1 space-y-6">
                        ...
                        <div className="space-y-6">
                            <div className="grid md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-zinc-500 uppercase px-1">Product Name</label>
                                    <input 
                                        name="name"
                                        required
                                        value={formData.name}
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-zinc-500 uppercase px-1">Category</label>
                                    <select 
                                        name="category"
                                        required
                    ...
                    <div className="bg-white dark:bg-zinc-900 p-6 md:p-8 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-zinc-500 uppercase px-1">Sale Price (₹)</label>
                                <input 
                                    type="number"
                                    name="price"
                                    required
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-zinc-500 uppercase px-1">Regular Price (₹)</label>
                                <input 
                                    type="number"
                                    name="originalPrice"
                    ...
                    <div className="bg-white dark:bg-zinc-900 p-6 md:p-8 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="font-bold text-lg dark:text-white">Product Images</h3>
                            </div>
                            <button type="button" onClick={addImageField} className="text-xs font-bold text-brand-primary hover:underline">+ Add More</button>
                        </div>
                                ...
                                <div key={idx} className="flex gap-2">
                                    <div className="flex-1 relative">
                                        ...
                                        <label className="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-zinc-400 hover:text-brand-primary cursor-pointer transition-colors">
                                            ...
                                            {uploading === idx ? <Loader2 size={18} className="animate-spin" /> : <Upload size={18} />}
                                        </label>
                                    </div>
                                    {formData.images.length > 1 && (
                                        <button type="button" onClick={() => removeImageField(idx)} className="w-12 h-12 flex items-center justify-center text-red-500 bg-red-50 dark:bg-red-500/10 rounded-xl border border-red-100 dark:border-red-500/20"><Trash2 size={18} /></button>
                                    )}
                                </div>
                            ))}
                        </div>
